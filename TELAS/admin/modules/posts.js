...
async function loadPosts() {
    ...
    try {
        ...
        if (error) {
            ...
            tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:#ef4444;padding:20px;">❌ Erro ao carregar posts: ${error.message}</td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = data.map(post => `
            <tr>
                ...
                <td>
                    <button class="btn-secondary" onclick="editarPost('${post.id}')" style="padding:4px 10px;margin-right:6px;" title="Editar">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn-danger" onclick="deletarPost('${post.id}')" style="padding:4px 10px;" title="Deletar">
