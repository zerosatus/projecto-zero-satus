// mobile-telas/script.js - VERSÃO SUPABASE APENAS
...
// ============================================
// CARREGAR DADOS (APENAS SUPABASE)
// ============================================
async function carregarTodosDados() {
    ...
    if (!usuarioLogado || !window.CacheManager) {
        console.warn('[Mobile] CacheManager ou usuário não disponível');
        return;
    }
    ...
    try {
        ...
        // Carregar todos os dados do CacheManager (que vem do Supabase)
...
// ============================================
// SALVAR DADOS (APENAS SUPABASE)
// ============================================
async function salvarTodosDados() {
    if (!usuarioLogado || !window.CacheManager || syncInProgress) {
        return false;
    }
    
    syncInProgress = true;
    console.log('[Mobile] 💾 Salvando dados no Supabase...');
    
    try {
        // Salvar no CacheManager (que sincroniza com Supabase)
        if (notes !== undefined) window.CacheManager.set('notes', notes, true);
        if (tasks !== undefined) window.CacheManager.set('tasks', tasks, true);
        if (calendarEvents !== undefined) window.CacheManager.set('calendarEvents', calendarEvents, true);
        if (weeklySchedule !== undefined) window.CacheManager.set('weeklySchedule', weeklySchedule, true);
        if (timeSlots !== undefined) window.CacheManager.set('timeSlots', timeSlots, true);
        if (notifications !== undefined) window.CacheManager.set('notifications', notifications, true);
...
function renderizarTudo() {
    console.log('[Mobile] 🔄 Renderizando UI...');
    renderizarHorario();
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('📱 Iniciando Mobile com Supabase...');
    ...
    if (window.CacheManager) {
        ...
        console.error('[Mobile] CacheManager não disponível!');
    ...
    console.log('✅ Mobile inicializado com Supabase!');
});

console.log('%c📱 Mobile - Supabase Apenas!', 'color: #10b981; font-size: 16px; font-weight: bold;');