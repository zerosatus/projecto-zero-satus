...
function getDefaultNotes() {
    return [
        { id: Date.now() - 1000, title: 'Fórmulas de Física', subject: 'Física', content: 'F = m * a', date: new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }), color: '#ef4444' }
    ];
}
...
function loadAllData() {
    ...
    weeklySchedule = window.getCached('weeklySchedule', getDefaultSchedule());
    timeSlots = window.getCached('timeSlots', ['08:00', '09:00', '10:00', '14:00']);
    ...
    if (window.onCacheChange) {
        window.onCacheChange('tasks', (newTasks) => { if (newTasks) { tasks = newTasks; refreshHomeData(); } });
        window.onCacheChange('calendarEvents', (newEvents) => { if (newEvents) { calendarEvents = newEvents; renderNextEvent(); } });
        window.onCacheChange('notes', (newNotes) => { if (newNotes) notes = newNotes; });
    }
}

function updateSummaryCards() {
    const materias = new Set();
    Object.values(weeklySchedule).forEach(day => { day.forEach(c => { if (c.materia) materias.add(c.materia.toLowerCase()); }); });
...
function closeEditModal() {
    const editModal = document.getElementById('edit-modal');
    if (editModal) { editModal.classList.remove('active'); renderSchedule(); }
}
        ...
        btn.addEventListener('click', (e) => {
            ...
            showConfirm(`Remover horário ${time}?`, 'Excluir Horário', (confirmed) => {
                if (confirmed) {
                    ...
                    days.forEach(day => { if (weeklySchedule[day]) weeklySchedule[day] = weeklySchedule[day].filter(c => c.horaInicio !== time); });
                    saveAllData();
                    updateSummaryCards();
                    renderEditSchedule();
...
function openSubjectModal(subject, day, time) {
    ...
    document.querySelectorAll('#subject-modal .color-option').forEach(option => { option.classList.toggle('active', option.dataset.color === selectedSubjectColor); });
...
function saveSubject() {
    ...
    weeklySchedule[day].push({ materia: name, professor: teacher, color: selectedSubjectColor, horaInicio: startTime, horaFim: endTime });
    ...
    const editModal = document.getElementById('edit-modal');
    if (editModal && editModal.classList.contains('active')) renderEditSchedule();
    else renderSchedule();
}
...
function switchView(viewName) {
    if (viewName === 'home') refreshHomeData();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('notification-bell-link')?.addEventListener('click', () => { document.getElementById('notifications-modal').classList.add('active'); renderNotificationsModal(); });
    ...
    document.getElementById('btn-cancel-time')?.addEventListener('click', () => { document.getElementById('new-time-input').value = '11:00'; });
    document.querySelector('[data-modal="subject-modal"]')?.addEventListener('click', () => { document.getElementById('subject-modal').classList.remove('active'); });
